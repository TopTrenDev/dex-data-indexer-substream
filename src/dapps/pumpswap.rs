...
pub fn parse_trade_instruction(
    ...
    if disc_bytes.len() < 8 {
        // Check length before unwrapping
        return None;
    }
        ...
        BUY_DISCRIMINATOR => {
            ...
            result = Some(TradeInstruction {
                ...
                amm: accounts
                    .get(0)
                    .map(|s| s.to_string())
                    .unwrap_or_default(), // Your index 0
                vault_a: accounts
                    .get(7)
                    .map(|s| s.to_string())
                    .unwrap_or_default(), // Your index 7
                vault_b: accounts
                    .get(8)
                    .map(|s| s.to_string())
                    .unwrap_or_default(), // Your index 8
                fee_account: Some(
                    accounts
                        .get(10)
                        .map(|s| s.to_string())
                        .unwrap_or_default()
                ), // Added fee_account with placeholder index 5
                ..Default::default()
            });
        }
        SELL_DISCRIMINATOR => {
